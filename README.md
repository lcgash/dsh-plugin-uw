...
在 DSH 中开发一个涉及多个目录的项目时，你常常面临两难：

- **只想给 AI 一个受限的目录范围**，不希望它越权访问系统里无关的目录（例如整个 `~/Projects`），也不想为每个子目录单独创建并来回切换会话。
- **但又确实需要同时访问多个目录**——典型场景是**前后端联调**：前端目录、后端目录、公共类型/协议目录放在不同的仓库里，AI 助手需要同时读写它们才能完成一次跨端修改。

如果只挂一个目录，AI 看不到其他目录的代码，无法完成跨端任务；如果挂一个大目录（比如整个主目录），又会造成**上下文爆炸**——AI 的工具扫描、文件检索会扩散到大量无关文件，既浪费 token 又降低回答质量，还授予了远超实际需要的目录权限。

**联合工作区**就是为这个痛点设计的：把**恰好需要的几个目录**合并进一个会话，AI 助手可以同时访问所有成员目录，但范围依然精确可控——它只看到你选定的目录，看不到范围外的任何文件。
...
- **两种权限预设**：
  ...
  | 预设 | 说明 |
  |---|---|
  | `workspace-write`（默认） | 主目录和成员目录均可读写（成员目录通过 `uw_write`/`uw_edit` 工具） |
...
- **成员目录工具** — 注册了 `uw_read`、`uw_write`、`uw_edit` 三个工具，AI 助手可直接通过成员目录路径读写文件，不受沙箱限制。所有成员目录均可读写。
...
6. AI 助手可通过 `uw_read` / `uw_write` / `uw_edit` 工具直接读写成员目录中的文件（标准 `read` / `write` / `edit` 工具受沙箱限制只能访问主目录）。
